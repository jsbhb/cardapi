...
CREATE TABLE `firecloud`.`member` (
  ...
  `isGuarantee` INT(1) NULL DEFAULT 0 COMMENT '是否平台担保',
  `isHighQuality` INT(1) NULL DEFAULT 0 COMMENT '是否优质商家',
  `isSincerity` INT(1) NULL DEFAULT 0 COMMENT '是否诚信示范',
  `isReturnGoods` INT(1) NULL DEFAULT 0 COMMENT '是否支持退换货',
...
CREATE TABLE `firecloud`.`commodity_show` (
  ...
  `commodityIntroduction` VARCHAR(200) NULL COMMENT '商品介绍',
  `packingSpecification` VARCHAR(200) NULL COMMENT '规格与包装',
  `instructions` VARCHAR(200) NULL COMMENT '使用说明',
  `customerService` VARCHAR(400) NULL COMMENT '购买及售后',
  `showPagePath` VARCHAR(200) NULL COMMENT '展示页面路径',
  `showPageSource` VARCHAR(2000) NULL COMMENT '展示页面代码',
  `createTime` DATETIME NULL COMMENT '创建时间',
  `createOpt` VARCHAR(50) NULL COMMENT '创建者',
  `updateTime` DATETIME NULL COMMENT '更新时间',
  `updateOpt` VARCHAR(50) NULL COMMENT '更新者',
